 ...
 */
import { ITEM_POOL, ITEM_STATE } from './itemPool.js';
/* Generated by the same script, kept in its own module so the main bundle can read the
   names without pulling the pool arrays out of the lazy Stats chunk. */
import { CUSTOM_ITEM_NAMES } from '../../config/customItems.js';

export { ITEM_POOL, ITEM_STATE, CUSTOM_ITEM_NAMES };
...
/*
 * Title-cased material name, unless a custom item owns that material — then it is the
 * custom item's name, because that is the only thing the game ever calls it. Six
 * materials are affected: NETHER_STAR is the Wheel of Fortune, KNOWLEDGE_BOOK the
 * Antimatter Locator, and so on. Title-casing them read as the vanilla item the player
 * never actually gets sent to find.
 *
 * The map is vendored from CustomMaterials.java by vendor-pool.mjs and already honours
 * the plugin's SHARES_MATERIAL_WITH_POOL_ITEM exclusions, so BRUSH still reads "Brush"
 * — it is a real EARLY force item that happens to share its material with the
 * Kiln-Fired Brush. Do not add entries here by hand; add them to the enum.
 */
export const itemLabel = (itemName) => {
    const key = itemKey(itemName);
    return CUSTOM_ITEM_NAMES[key.toUpperCase()] ?? titleCase(key);
};

/*
 * Everything a search should match for an item: its shown label plus its raw material
 * name, so a custom item stays findable by the material it rides on ("nether star"
 * still finds the Wheel of Fortune). Identical for every non-custom item, where the
 * label IS the title-cased material.
 */
export const itemSearchText = (itemName) => `${itemLabel(itemName)} ${titleCase(itemKey(itemName))}`;
