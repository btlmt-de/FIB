...
import { Section, Avatar, Medal, Sprite, Empty, Figure, RarityTag } from './Primitives.jsx';
import { Inventory } from './Inventory.jsx';
import { RaceTrace } from './Charts.jsx';
import * as f from './format.js';

const labelFor = (row) => row.members.map((uuid) => playerName(uuid)).join(' & ');
...
export function MatchDetail({ match, onBack, onOpenPlayer }) {
        ...
        <RaceTrace
          ...
          iconFor={(entry) => (
            <span className="fib-lane-faces" aria-hidden="true">
              {entry.members.map((uuid) => (
                <Avatar key={uuid} uuid={uuid} size={16} />
              ))}
            </span>
          )}
        />
                      ...
                      <td>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--fib-space-3)', flexWrap: 'wrap' }}>
                          {row.entry.members.map((uuid) => (
                            ...
                            >
                              <Avatar uuid={uuid} size={24} />
                              <span>{playerName(uuid)}</span>
                            </button>
                          ))}
                        </div>
                      </td>
