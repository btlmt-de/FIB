/*
 * ═══════════════════════════════════════════════════════════════════════════
 * THE ARRIVAL — the reel closing, and what pulls in
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * Two layers over the reel mount, in this order:
 *
 *   1. THE SHUTTER, canvas 2D. For the first beat this layer is transparent
 *      except for the blades, so the player watches their OWN wheel get closed
 *      down slot by slot rather than cut to a picture of one. After that it is
 *      the dark band the platform stands on, and at the end it lifts and the
 *      reel — which never stopped rendering underneath — is simply revealed.
 *
 *   2. THE TRAIN, WebGL. See ArrivalTrain3D.jsx, including why the locomotive
 *      is modelled in code rather than downloaded.
 *
 * They are separate because they are doing different jobs: the shutter belongs
 * to the reel's own 2D geometry (a 120px slot pitch), and the train belongs to a
 * world with perspective in it. Trying to draw the shutter inside the 3D scene
 * would put the reel's slots into world space and give them vanishing points,
 * which is exactly what they must not have.
 *
 * DESIGN.md §8 calls the reel "a main transit line arriving at its stop". The
 * band is the line; this is the arrival.
 */

import React, { useEffect, useRef, lazy, Suspense } from 'react';
import { rail } from '../config/constants';
import { prefersReducedMotion } from '../../../utils/motion.js';
import { T_SHUTTER, T_LIFT, T_LIFT_END, SCENE_FADE_S } from './arrivalTimeline.js';

/*
 * three.js is fetched WHEN A TRAIN ARRIVES, not when the page loads.
 *
 * Statically imported it added 132KB gzipped to the wheel chunk — 140KB to 272KB,
 * very nearly doubling the heaviest route on a site PRODUCT.md describes as "a
 * phone beside a running game". Arrivals fire roughly twice a day, so almost
 * every visit was paying for a locomotive it would never see.
 *
...
export function ArrivalTrain({ crateCount = 0 }) {
    ...
    useEffect(() => {
        ...
        const frame = (now) => {
            ...
            const OPEN_START    = T_LIFT + SCENE_FADE_S;
            const OPEN_SPAN     = Math.max(0.1, T_LIFT_END - OPEN_START);
            ...
            for (let i = 0; i < slots; i++) {
                ...
                const openAge = motionOff ? -1 : t - (OPEN_START + openLead);
    ...
    return (
        ...
        >
            ...
            {/*
              * The train fades up as the last blade lands. It is a separate
              * canvas because it is a separate renderer, and it sits above the
              * shutter because the platform stands on the closed band.
              */}
            <Suspense fallback={null}>
                <ArrivalTrain3D
                    crateCount={crateCount}
                    style={{
                        animation: `fadeIn 0.5s ease-out ${T_SHUTTER * 0.75}s both`,
                    }}
                />
            </Suspense>
        </div>
